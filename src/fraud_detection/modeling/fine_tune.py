...
def run_fine_tuning_pipeline(n_trials: int = 15) -> dict[str, Any]:
    ...
    lr_baseline_metrics = _evaluate_on_splits(
        baseline_lr, X_train, y_train, X_val, y_val, X_test, y_test
    )
    lgbm_baseline_metrics = _evaluate_on_splits(
        baseline_lgbm, X_train, y_train, X_val, y_val, X_test, y_test
    )
    lr_tuned_metrics = _evaluate_on_splits(lr_model, X_train, y_train, X_val, y_val, X_test, y_test)
    lgbm_tuned_metrics = _evaluate_on_splits(
        lgbm_model, X_train, y_train, X_val, y_val, X_test, y_test
    )
    ...
    lr_payload = {
        ...
        "test_auprc_improvement": float(
            lr_tuned_metrics["test"]["auprc"] - lr_baseline_metrics["test"]["auprc"]
        ),
    }
    lgbm_payload = {
        "model_type": "LightGBM",
        ...
        "test_auprc_improvement": float(
            lgbm_tuned_metrics["test"]["auprc"] - lgbm_baseline_metrics["test"]["auprc"]
        ),
    }
    ...
    best_model_name = (
        "LGBM Tuned"
        if lgbm_tuned_metrics["test"]["auprc"] >= lr_tuned_metrics["test"]["auprc"]
        else "LR Tuned"
    )
    best_model_auprc = (
        lgbm_tuned_metrics["test"]["auprc"]
        if best_model_name == "LGBM Tuned"
        else lr_tuned_metrics["test"]["auprc"]
    )
    summary_text = (
        "HYPERPARAMETER TUNING SUMMARY\n"
        f"Trials per model: {n_trials}\n"
        f"LR test AUPRC: baseline={lr_baseline_metrics['test']['auprc']:.4f}, "
        f"tuned={lr_tuned_metrics['test']['auprc']:.4f}\n"
        f"LGBM test AUPRC: baseline={lgbm_baseline_metrics['test']['auprc']:.4f}, "
        f"tuned={lgbm_tuned_metrics['test']['auprc']:.4f}\n"
