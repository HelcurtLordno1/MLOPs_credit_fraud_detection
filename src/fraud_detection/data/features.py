...
def engineer_customer_velocity_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cc_stats = df.groupby("cc_num").agg({
        "cc_num": "count",
        "amt": ["mean", "std", "sum"],
        "is_fraud": ["sum", "mean"],
    }).reset_index()
    
    cc_stats.columns = ["cc_num", "customer_txn_count", "customer_avg_amt", 
                         "customer_std_amt", "customer_total_amt", 
    ...
    df = df.merge(cc_stats[["cc_num", "customer_txn_count", "customer_avg_amt", 
                              "customer_std_amt", "customer_fraud_rate"]], 
                   on="cc_num", how="left")
...
def engineer_merchant_risk_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    merchant_stats.columns = ["merchant", "merchant_txn_count", "merchant_avg_amt",
                               "merchant_std_amt", "merchant_fraud_count", "merchant_fraud_rate"]
    ...
    # Merge back to original dataframe
    df = df.merge(merchant_stats[["merchant", "merchant_fraud_rate", "merchant_txn_count",
...
def engineer_category_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_stats.columns = ["category", "category_txn_count", "category_avg_amt",
                          "category_fraud_count", "category_fraud_rate"]
    
    cat_stats["category_fraud_rate"] = cat_stats["category_fraud_rate"].fillna(0)
    
    # Merge back to original dataframe
    df = df.merge(cat_stats[["category", "category_fraud_rate", "category_txn_count",
                               "category_avg_amt"]], 
...
def get_feature_names() -> dict[str, list[str]]:
    ...
    return {
        ...
        "customer_velocity": [
            "customer_txn_count", "customer_avg_amt", "customer_std_amt", "customer_fraud_rate"
        ],
        "merchant_risk": [
            "merchant_fraud_rate", "merchant_txn_count", "merchant_avg_amt", "merchant_std_amt"
        ],
        "category": [
            "category_fraud_rate", "category_txn_count", "category_avg_amt"
        ],
        "distance": [
            "distance_km", "is_distant_txn"
        ],
        "amount": [
            "amt_zscore", "amt_is_outlier", "amt_ratio_to_customer_avg"
        ],
        "raw_numeric": [
